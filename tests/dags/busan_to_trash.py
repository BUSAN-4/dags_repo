...
@dag(
    dag_id='busan_to_trash',
    description='busan_car → trash DB 데이터 전송 (5개씩 순차)',
    schedule='*/1 * * * *',  # 1분마다 실행
    start_date=datetime(2025, 12, 16, tzinfo=KST),
    catchup=False,
    tags=['flink', 'batch', 'busan', 'trash', 'migration'],
    default_args={
        'owner': 'airflow',
...
def busan_to_trash():
    ...
    def calculate_offset(**context):
        ...
        try:
            current_offset = int(Variable.get('busan_to_trash_offset', default_var='0'))
        except:
            current_offset = 0
            Variable.set('busan_to_trash_offset', '0')
        
        logger.info(f"📊 현재 offset: {current_offset} (행 {current_offset+1}~{current_offset+5})")
        
        # 다음 실행을 위해 offset 증가
        next_offset = current_offset + 5
        Variable.set('busan_to_trash_offset', str(next_offset))
    ...
    def read_sql_file(offset: int):
        ...
        sql_file_path = "/opt/airflow/dags/repo/flink_sql/05_busan_to_trash.sql"
    ...
    def submit_batch_job(sql_content: str):
        ...
        try:
            ...
            logger.info("✅ 모든 SQL 실행 완료 - busan_car → trash 전송 성공")
...
# DAG 인스턴스 생성
busan_to_trash_dag = busan_to_trash()
